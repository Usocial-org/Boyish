const state = { content: { pictures: [], videos: [], blogs: [], links: [] }, adminToken: sessionStorage.getItem('boyishToken') || null };
...
// Keep the adult confirmation until the visitor explicitly verifies their age.
const ageGate = $('#ageGate');
if (localStorage.getItem('boyishAgeVerified') === 'true') {
  ageGate.classList.remove('show');
}
$('#ageGateYes').addEventListener('click', ()=>{
  localStorage.setItem('boyishAgeVerified', 'true');
  ageGate.classList.remove('show');
});
$('#ageGateNo').addEventListener('click', ()=>{
  $('#ageGateError').textContent = 'You must be 18 or older to enter Boyish.';
});
...
function showSection(section, updateHistory = true){
  ...
  document.title = section === 'discover' ? 'Boyish' : `${section === 'blog' ? 'Journal' : section[0].toUpperCase() + section.slice(1)} | Boyish`;
}
...
function videoMarkup(video, preview = false){
  ...
  return `<iframe src="${escAttr(source)}" loading="lazy" title="${escAttr(video.title || 'Boyish video')}" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>`;
}
...
function interactionBar(type, id){
  const liked = localStorage.getItem(`boyishLike:${type}:${id}`) === 'true';
...
function renderDetail(type, id){
  ...
  document.title = `${item.title || item.caption || 'Picture'} | Boyish`;
...
async function handleDetailAction(event){
  ...
  const item = type === 'picture'
    ...
    : type === 'video'
      ? state.content.videos.find(entry => entry.id === id)
      : state.content.blogs.find(entry => entry.id === id);
  const title = item?.title || item?.caption || 'Boyish collection';
  const shareData = { title, text: `View ${title} on Boyish`, url: window.location.href };
  try {
...
function openAdminEntry(){
  window.open('/admin.html', 'boyishAdmin');
}
...
async function submitLogin(){
  ...
  try{
    ...
    sessionStorage.setItem('boyishToken', token);
...
$('#logoutBtn').addEventListener('click', async ()=>{
  ...
  sessionStorage.removeItem('boyishToken');
